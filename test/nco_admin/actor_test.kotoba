(ns nco-admin.actor-test
  "Integration tests for `nco-admin.actor` — builds the REAL compiled
  `langgraph.graph` and runs `run-request!`/`approve!` end-to-end
  through all three terminal routes (commit / escalate-then-approve /
  hold). This namespace did not exist before: `build-graph` previously
  called `graph/state-graph-builder`, a function that does not exist
  anywhere in `kotoba-lang/langgraph`, and `run-request!` was an
  explicit stub — both went uncaught for as long as they did precisely
  because no test ever exercised this namespace. These tests close
  that gap and prove the audit ledger (`nco-admin.store/add-record!`)
  is genuinely wired into the `:commit`/`:hold` nodes."
  (:require [clojure.test :refer [deftest is testing]]
            [nco-admin.actor :as actor]
...
(deftest run-request-commits-clean-proposal
  (testing "a valid, high-confidence, nominal readiness report runs the
    ...
    (let [st (fresh-store)
          ...
          result (actor/run-request! g {:nco-id "nco-001" :type :log-readiness
      ...
      (testing "the commit is genuinely durable in the store's real audit
                ledger (`nco-admin.store/add-record!`), not just the
                transient graph-state `:records` mirror"
        (let [ledger (store/records (:store state))]
          (is (= 1 (count ledger)))
          (is (= :log-readiness-report (:type (first ledger))))
          (is (= "nco-001" (:nco-id (first ledger)))))))))

(deftest run-request-holds-unregistered-nco
  (testing "an unregistered NCO is a HARD violation -- the real graph
            routes to :hold and terminates, never :commit"
    (let [st (fresh-store)
          g (actor/build-graph (advisor/mock-advisor) st)
          result (actor/run-request! g {:nco-id "no-such-nco"
      ...
      (testing "the HARD violation is ALSO durably recorded to the real
        ...
        (let [ledger (store/records (:store state))]
          ...
          (is (= "no-such-nco" (:nco-id (first ledger))))
...
(deftest run-request-escalates-then-approve-commits
  (testing "draft-correspondence always escalates -- the real graph
    ...
    (let [st (fresh-store)
          g (actor/build-graph (advisor/mock-advisor) st)
          held (actor/run-request! g {:nco-id "nco-001"
...
(deftest run-request-low-confidence-forbidden-op-holds
  (testing "confidence below the governor's floor also escalates in
    ...
    (let [st (fresh-store)
          g (actor/build-graph (advisor/mock-advisor) st)
          result (actor/run-request! g {:nco-id "nco-001" :type :bogus}
                                      {} "thread-bogus-1")
          state (:state result)]
      (is (= :done (:status result)))
      (is (true? (:hard? (:decision state))))
