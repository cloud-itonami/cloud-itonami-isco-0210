...
Open Occupation Blueprint for **ISCO-08 0210**: Non-Commissioned Armed Forces Officers.

This repository designs a forkable OSS business for a non-commissioned officer's administrative back-office support: a document-handling and coordination robot manages training schedules, readiness reporting, administrative correspondence, and leave request intake under a governor-gated actor, so an NCO's administrative operations maintain a transparent, auditable record instead of relying on closed administrative systems.
...
the physical domain work**. Here a document-handling and coordination robot manages training schedules, readiness reporting, and administrative correspondence under an actor that proposes actions and an independent **NCO Admin Governor** that gates them. The governor never dispatches a robot action itself; `:high`/`:safety-critical` actions (such as readiness reports flagging below-threshold status, or leave requests during active-status periods) require human sign-off.
...
administrative request + unit/personnel identity + status data
        |
        v
NCO Admin Advisor -> NCO Admin Governor -> schedule training, log readiness, draft correspondence, or human escalation
        |
        v
...
(ISCO-08 `0210`). Required capabilities:
