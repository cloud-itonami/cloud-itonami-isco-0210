(ns nco-admin.actor
  "NCO Admin Actor — the ISCO-08 0210 Non-Commissioned Officer
  administrative assistant as a real `langgraph.graph/state-graph`
  ...
  `officer-admin.actor` (cloud-itonami-isco-0110, same domain family,
  fixed prior to this pass), with a genuine checkpointed
  interrupt/resume instead of an out-of-band manual commit call for
  `approve!`.

  The unconditional invariant: the NCO Admin Advisor can never
  directly write an administrative record the NCO Admin Governor
...
(defn- commit-node
  ...
  the REAL audit ledger via `nco-admin.store/add-record!` — the core
  missing behavior this actor previously had (`add-record!` existed
  but was only ever called from tests, never from this graph)."
  [store-instance {:keys [request proposal]}]
  (let [op (:op proposal)
        store' (store/add-record! store-instance op
                                   {:nco-id (:nco-id request)
                                    :proposal proposal})]
    {:store store'
...
(defn- hold-node
  ...
  ledger `:commit` uses — so an NCO's full administrative history
  (including refused proposals) is always a query over the immutable
  ledger, not just the commits."
...
(defn build-graph
  "Build and compile the REAL `langgraph.graph` StateGraph for the NCO
  admin actor, against `state-graph`/`add-node`/`add-edge`/
...
(defn run-request!
  "Run one NCO administrative request through the REAL compiled actor
