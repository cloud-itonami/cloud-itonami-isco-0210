(ns nco-admin.governor
  "NCOAdminGovernor — the independent safety/traceability layer for
  the ISCO-08 0210 Non-Commissioned Officer administrative assistant actor.
  Wired as its own `:govern` node in `nco-admin.actor`'s StateGraph,
  downstream of `:advise` — the Advisor has no notion of NCO provenance
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    1. nco provenance          — the request's NCO must be registered.
  ...
  (:require [nco-admin.store :as store]))
...
(defn- hard-violations [{:keys [proposal]} nco-record]
  (cond-> []
    (nil? nco-record)
    (conj {:rule :no-nco :detail "NCO not registered"})
...
(defn- is-active-status? [nco-record]
  ; Simplified: active-duty NCOs have certain restrictions on leave
  ; In real implementation, would check duty status, deployment calendar, etc.
  (get nco-record :active-duty false))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `nco-admin.store/Store`. Returns
  `{:ok? bool :violations [...] :confidence n :hard? bool :escalate? bool}`."
  [request context proposal store]
  (let [nco-record (store/nco store (:nco-id request))
        hard (hard-violations {:proposal proposal} nco-record)
